...
python $CURDIR/scripts/main.py \
 --config configs/datasets/ami/w-europe_novelbranch_20.yml   \
   $CURDIR/configs/networks/resnet50.yml \
    ...
    --wandb.name novel_branch_onehead_w-europe \
    --network.name resnet50 \
    --network.pretrained True \
    --network.checkpoint /network/scratch/y/yuyan.chen/ood_benchmark/pretrained_models/model_5749458.pth \
    --optimizer.num_epochs 30 \
    --optimizer.warmup_epochs 2 \
    --optimizer.lr 0.001 \
    --run_dir ood_benchmark/weights/novel_branch/ami/w-europe \
