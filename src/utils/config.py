        ...
        for key in subconf.keys():
            ...
            if type(value) is str and "@" in value:
                if value.count("@") == 1 and value.startswith("@{") and value.endswith("}"):
                    ...
                    origin_ref_key_list = ["'@{" + ref_key + "}'" for ref_key in ref_key_list]
                    for origin_ref_key, ref_value in zip(origin_ref_key_list, ref_value_list):
    ...
    for i in range(len(configs)):
        ...
        if not isinstance(config, Config):
            raise TypeError(
                ...
                "Incorrect usage: merge_configs([configs1, configs2, ...])".format(type(config))
            )
...
def consume_dots(config, key, create_default):
    ...
    if sub_key in Config.__dict__:
        raise KeyError(
            '"{}" is a preserved API name, ' "which should not be used as normal dictionary key".format(sub_key)
        )
