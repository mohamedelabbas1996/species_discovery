...
def get_network(network_config):
    ...
    num_classes = int(network_config.num_classes)
    ...
    # if network_config.name == "mobilenet":
    #     net = MobileNet(num_classes=num_classes)
    if network_config.name == "extended_net":
        backbone = get_network(network_config.backbone)
        backbone.fc = nn.Identity()
        net = ExtendedNet(backbone, num_classes, network_config.num_all_classes)
    ...
    #                 subnet.to(device), device_ids=[comm.get_local_rank()], broadcast_buffers=True
    #             )
    #     else:
    #         net = torch.nn.parallel.DistributedDataParallel(
    #             net.to(device), device_ids=[comm.get_local_rank()], broadcast_buffers=True
    #         )
