...
def test_kmeans(k, data_dict, config):
    """
    In this case, the test loader needs to have the labelled and unlabelled subsets of the training data
    """

    all_feats = np.concatenate([data_dict["val"]["feat_list"], data_dict["test"]["feat_list"]])
    ...
    labelled_acc = cluster_acc(data_dict["val"]["label_list"].astype(int), preds.astype(int))

    return labelled_acc


def test_kmeans_for_scipy(k, data_dict):
    """
    In this case, the test loader needs to have the labelled and unlabelled subsets of the training data
    """
...
def binary_search(data_dict, config):
    ...
    labelled_acc_big = test_kmeans(big_k, data_dict, config)
    labelled_acc_small = test_kmeans(small_k, data_dict, config)
    labelled_acc_middle = test_kmeans(middle_k, data_dict, config)
...
def scipy_optimise(data_dict, config):
    ...
    test_k_means_partial = partial(test_kmeans_for_scipy, data_dict)
